...
function interior(context: CanvasRenderingContext2D, char: string, font: string): Omit<Ink, "index"> | null {
  ...
  for (let y = 0; y < height; y++) {
    ...
    for (let x = 0; x < width; x++) {
      const above = rows[x + 1] ?? 0;
      const run = (pixels[(y * width + x) * 4 + 3] ?? 0) > 245
        ? Math.min(above, rows[x] ?? 0, diagonal) + 1 : 0;
      rows[x + 1] = run;
      diagonal = above;
      if (run > size) { size = run; bx = x; by = y; }
    }
  }
...
export default function GlyphPortal({
  ...
  // Each character with its index in the string, counted in code units.
  const characters = Array.from(text).reduce<{ char: string; index: number }[]>((list, char) => {
    const last = list.at(-1);
    list.push({ char, index: last ? last.index + last.char.length : 0 });
    return list;
  }, []);
  ...
  const hasFront = front !== null && front !== undefined;
  const q = `:where(#${uid})`;
  ...
  useLayoutEffect(() => {
    ...
    const readInk = () => {
      ...
      for (const char of Array.from(text)) {
        context.font = `${font.fontWeight} 100px ${font.fontFamily}`;
        const m = context.measureText(char);
        const advance = advances[offset] ?? 0;
        letters.push({ index: offset, x: advance - m.actualBoundingBoxLeft,
        ...
        if (found) candidates.push({ ...found, x: found.x + advance, index: offset });
        offset += char.length;
      }
